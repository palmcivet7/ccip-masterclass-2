...
contract CrossChainReceiver is CCIPReceiver, OwnerIsCreator {
    ...
    constructor(address ccipRouterAddress, address simplifiedStakingAddress) CCIPReceiver(ccipRouterAddress) {
    ...
    function _ccipReceive(Client.Any2EVMMessage memory any2EvmMessage) internal override {
        IERC20(any2EvmMessage.destTokenAmounts[0].token).approve(
            address(i_simplifiedStaking), any2EvmMessage.destTokenAmounts[0].amount
        );

        i_simplifiedStaking.stake(any2EvmMessage.destTokenAmounts[0].amount, abi.decode(any2EvmMessage.data, (address)));
    }
